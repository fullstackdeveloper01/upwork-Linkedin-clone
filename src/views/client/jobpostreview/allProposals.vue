<template>
  <div class="search-filter-wrapper border-bottom br-20 bg-white freelancer-wrap mt-12">
    <b-row>
      <b-col xl="12" lg="12" md="12" class="pb-0">
        <FilterReviewproposal :searchData="searchData" :pageName="'allproposal'" @filterByData="getProposalData"/>
        <template v-if="proposalData.length > 0 ">
          <template v-for="(proposal, id) in proposalData" >
            <div class="profile-wrap mb-0 px-4 pt-0 py-4 pos-rel job-card" :key="id" :id="id" :class="(proposal.seen == 0)?'proposal-unread':''"  :if="proposal.application_status == 2"  v-on:click="read(proposal,id)">
              <b-row>
                  ...
                  <div class="d-md-flex align-items-center mb-2">
                    ...
                    <div class="content">
                      ...
                      <ul class="align-items-center fz-14 post text-muted mb-0" v-if="proposal.tagline">
                        <li class="fz-16 text-black ml-1" >{{proposal.tagline}}</li>
                      </ul>
                ...
                <b-col cols="4" class="text-right d-flex align-items-center justify-content-end pb-0">
                  <div class="action-btn d-flex align-items-center top-13">
                    ...
                    <button type="button" class="btn btn-outline-secondary br-50 mr-3 btn-like" :class="(proposal.application_status == 3)?'saved-btn':''" @click="proposalLike(proposal,3)">
        ...
        <div class="text-center px-4 py-4" v-else>
          ...
          <template v-else>
            ...
            <h4 class="fz-20 fw-600 text-black">You don't have any Proposal yet</h4>
            <p class="fz-14 fw-500 text-black mb-0">Interview promising candidates and make them an offer.</p>
          </template>
          
        </div>
        <MessageDetails v-if="modelopenclose" ref="handleMessageDetailspage" :jobDetails='jobDetails' :jobId="jobId" :messageData="messageData"  @getProposalData="getProposalData" @applicationStatus="updatecountStatus" @modelopenclose="modelopenclose=false"/>
      </b-col>
    </b-row>
  </div>
</template>
<script>
  ...
  export default {
    ...
    computed:{
      filterData(){
        return this.proposalData.filter(function (item) {
            // return item.newSkill_name.find(function (c){
            item.newSkill_name.map((m) => [m, m])
               return item;
            // })
        })
      },
    ...
    methods: {
      ...
      async read(proposal,_i){
        await this.genericAPIPOSTRequest({
            requestUrl: "readProposals",
            params: { 
              "job_id": proposal.jobs_id,
              "user_id":proposal.user_id
            }
          });
        document.getElementById(_i).classList.remove("proposal-unread")
