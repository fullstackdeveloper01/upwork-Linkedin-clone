  ...
  <div class="search-filter-wrapper border-bottom pb-0 br-20 bg-white freelancer-wrap mt-12">
    <b-row>
      <b-col xl="12" lg="12" md="12" class="pb-0">
        <FilterReviewproposal :searchData="searchData" :pageName="'messaged'" @filterByData="getProposalData"/>
          ...
          <b-row class="profile-wrap mb-0 px-4 pt-0 py-4 pos-rel job-card" v-for="(proposal, id) in proposalData" :key="id">
            ...
            <b-col cols="4" class="text-right d-flex align-items-center justify-content-end pb-0">
              <div class="action-btn d-flex align-items-center top-13">
                ...
                <span v-if="proposal.application_status != 2 && proposal.application_status != 7 && proposal.application_status != 8">
                ...
                <!-- <a class="btn btn-theme" target="_blank" :href="$router.resolve({name:'hiredetails',path:'/client/hire-details',params:{ pid:proposal.uniq_id,uid:proposal.user_id}}).href">Hire</a> -->
              </div>
            </b-col>
        ...
        <template v-else>
          ...
          <div class="text-center  px-4 py-4" v-else>
            ...
            <h4 class="fz-20 fw-600 text-black">You don't have any Messages yet</h4>
  ...
  export default {
    ...
    methods: {
      ...
      SlideSkill(id,direction,_i){
        ...
        if(direction =='right'){
          if(this.sliderScrollDatas[_i] > 0){
            this.sliderScrollDatas[_i] = this.sliderScrollDatas[_i]+parseInt(slider_width);
          }else{
            this.sliderScrollDatas[_i] = parseInt(slider_width);
          }
          if(main_width > this.sliderScrollDatas[_i]){
            const conent = document.querySelector('#'+id);
            conent.scrollLeft += slider_width;
            // console.log('sliderScrollDatas',this.sliderScrollDatas[_i]);
            // console.log('main_width',main_width);
            if(this.sliderScrollDatas[_i] == main_width){
              document.getElementById('next_'+_i).style.display = 'none';
            }
            document.getElementById('prev_'+_i).style.display = 'block';
          }else if(this.sliderScrollDatas[_i] >= main_width){
            this.sliderScrollDatas[_i] = this.sliderScrollDatas[_i]-parseInt(slider_width);
            document.getElementById('next_'+_i).style.display = 'none';
          }
        }else{
          ...
          }else if(this.sliderScrollDatas[_i] < 0){
            document.getElementById('prev_'+_i).style.display = 'none';
            this.sliderScrollDatas[_i] = this.sliderScrollDatas[_i]+parseInt(slider_width);
          }
...
<style>
  #readmore{color:red;float: right;}
</style>
