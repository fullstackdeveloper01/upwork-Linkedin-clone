  ...
  <b-container>
    ...
    <b-row class="mt-100 hire-wrap">
      <b-col xl="12" lg="12" sm="12">
                ...
                <div class="ml-6 ml-sm-0 mt-sm-4 width-80">
                  ...
                  <div class="hire-profile-details d-flex align-items-flex-start ml-2 ml-sm-0 mt-8">
                    <div class="d-flex align-items-center mr-6">
                      <img src="http://qapin_mvp5.manageprojects.in/upload/location.png" class="img-fluid mr-2 w-12">
                      <h5 class="mb-0">{{userDetails.country_name+', '+userDetails.state_name+', '+userDetails.city_name}}</h5>
                      <!-- <h6>1:26 pm local time</h6> -->
                    </div>
                    <div class="d-flex align-items-center">
                      <img src="http://qapin_mvp5.manageprojects.in/upload/job-success.png" class="img-fluid mr-2 w-12">
                      <h5 class="mb-0">{{userDetails.jobSuccessRate}}% Job Success</h5>
                    </div>
                    <!-- <div class="d-flex align-items-center">
                      <img src="http://qapin_mvp5.manageprojects.in/upload/top-rated.png" class="img-fluid w-12 mr-2">
        ...
        <form @submit.prevent="fundedMilestone('submitProposal')">
          <b-card no-body>
            <div class="view-wrapper mb-4">
              <div>
                ...
                <div class="px-4 py-4">
                  ...
                  <!-- <h4 class="fz-18 fw-500 text-black mb-4">Deposit funds into Escrow</h4>

                  <p class="fz-16 fw-600 mb-4 text-muted">Escrow is a neutral holding place that protects your deposit until work is approved.</p>

                  <b-form-group>
                    <b-form-checkbox-group
                      v-model="selected"
                      :options="options"
                      stacked class="fz-16 fw-600"
                    ></b-form-checkbox-group>
                  </b-form-group> -->
          ...
          <b-card no-body>
            <div class="view-wrapper mb-4">
              <div>
                ...
                <div class="px-4 py-4">
                  <h4 class="fz-20 fw-600">Related Job Listing</h4>
                  <h4 class="fz-18 fw-400 mb-4"><a href="javascript:void(0);" class="text-theme" @click="$router.push({name:'jobpostreview/job-details',path:'/client/jobpostreview/job-details',params:{'pid':$route.params.pid}})">{{jobDetails.job_title}}</a> <!--(#1000280820)--></h4>
                  <div class="contract-title-wrap">
                    <div class="d-md-flex align-items-center">
                      <h4 class="fz-20 fw-600">Contract Title</h4>
                      <div class="ml-auto">
                        <a href="javascript:void(0);" @click="titleEdit = (titleEdit==true)?false:true">
                    ...
                    <h4 class="fz-18 fw-400 mb-4 text-muted" v-if="!titleEdit">{{userJobApply.title}}</h4>
                    <b-form-input v-model="userJobApply.title" v-else size="sm" placeholder="Enter Job Title" class="inp-person-first-name custom-select"></b-form-input>
                    <p v-if="titleEdit && userJobApply.title ==''" style="color:red" class="error">Title Is required</p>
                  </div>
                  ...
                  <div class="contract-title-wrap">
                    ...
                    <p class="fz-18 fw-400 mb-4 text-black text-muted" v-if="!desEdit">{{userJobApply.description}}</p>
                    <b-form-textarea
                    v-else
                    id="textarea"
                    v-model="userJobApply.description"
                    placeholder="Write somthing here..."
                    rows="3"
                    max-rows="6"
                    ></b-form-textarea>
                    <p v-if="titleEdit && userJobApply.description ==''" style="color:red" class="error">Description Is required</p>
                  </div>
                  ...
                  <div class="contarct-description-wrapper">
                    <div class=" align-items-center">
                      ...
                      <div>
                        <ul  class="ml-2 pl-0 mt-4 attachment "  v-if="userJobApply.attachment.length > 0" style="padding: 6px 0px;">
                          <template v-for="(attc, index) in userJobApply.attachment">
                            <li class="attatchment-wrap mr-5 mb-2"  :key="index" v-if="index >= 0" style="border-radius: 20px;padding: 2px 10px;margin-right: 5px;">
                              ...
                              <a :href="userJobApply.attachmentUrl+attc" target="_blank"><span class="ml-2 text-theme">{{attc}}</span></a>
              ...
              <div>
                <div class="px-4 py-4">
                  <b-form-group class="d-flex align-items-center">
                    ...
                    <!-- <div class="error term_condition"  v-if="term_condition">Check is required.</div> -->
                  </b-form-group>
                ...
                <div class="footer px-4 py-4">
                  <div class="text-right">
                    ...
                    <span v-if="canEditMilestone">
                      <b-button class="btn btn-theme" v-if="milstoneChangestatus" @click="fundedMilestone('SendOffer');">Accept</b-button>
                      <b-button type="submit" class="btn btn-theme" v-else>Continue</b-button>
                    </span>
                  </div>
                </div>
              </div>
        ...
        <b-modal  id="payment-modal" size="xl" title="Payment Details" centered hide-footer no-close-on-backdrop >
          <template #modal-header>
              <div class="w-100 text-right">
                  <a href="javascript:void(0);" class="text-theme mr-2" @click="closePaymentModal(); offerTheUser=false"><i class="fa fa-close text-theme"></i></a>
              </div>
          </template>
          <Paymentgateway ref="childComponentfun" :contractDetails="contractDetails={'userDetails':userDetails,'jobDetails':jobDetails}" :currentpayment_arr="currentpayment_arr" @hideLoader="loaderHideshow" @closeModal="closePaymentModal" @milseStoneDatarefresh="milseStoneData" @paymentResponse="paymentResponse" @thankYouModal="$bvModal.show('thankyou-modal')"></Paymentgateway>
        ...
        <b-modal id="thankyou-modal" title="Payment Successfull" centered hide-footer size="md" no-close-on-backdrop> 
            ...
            <div>
                <div class="text-right">
                    <a href="javascript:void(0);" class="text-theme mr-2" @click="$bvModal.hide('thankyou-modal')">close</a>
                </div>
            </div>
            </template>
        </b-modal>
  ...
  export default {
    ...
    data() {
      return {
        ...
        options: [
          { text: 'Deposit $1600.00 for the whole project.', value: 'a' },
          { text: 'Deposit a lesser amount to cover the first milestone.', value: 'b' },
        ],
        privacyoptions:0,
        jobsuccessRate:0,
        userJobApply:{
    ...
    methods:{
      ...
      // THIS FUNCTION IS FOR TO RECIVED PAYMENT RESPONSE FROM PAYMENT GATEWAY
      paymentResponse(_v){
        this.offerTheUser=_v;
        if(this.offerTheUser == true){
          if(this.submitType == 'SendOffer'){
            this.SendOffer(7);
          }else{
            this.submitProposal();
          }
      ...
      // this fun is for to get job success Rate
      ...
      CheckTermsCondition(){
        if(this.privacyoptions != 1) this.term_condition=true;
        ...
        if(ext=="pdf" || ext=="word" || ext=="png"|| ext=="jpeg"|| ext=="jpg"){
          ...
          // if (fileSize >= 2 && fileSize <= 40) {
            ...
            if(getData.filename!=""){
              ...
              this.userJobApply.attachment.push(getData.name)
      ...
      // this fun is for to show payment gateway 
      async fundedMilestone(_v){
        this.submitType = _v;
        if(this.submitType == 'submitProposal'){
          this.errorMessageshow = true;
          this.$v.proposalData.$touch(); 
          if (this.$v.proposalData.$invalid ) {
            this.term_condition=false;
            this.$toastr.e('Please Fill Required Fields');
            return false;
          }else if(this.privacyoptions != 1){
            this.term_condition=true;
            this.$toastr.e(' Terms & Policy is Required');
            return false;
          }
          ...
          if(response){
            this.getMilestonedetails(this.user_id);
            setTimeout(() => {
              this.currentpayment_arr = this.proposalData[0];
            ...
            this.$toastr.e('Offer Not Sent');
            this.$router.push({ name: 'index', path:'/client/jobpostreview/', params: {pid: this.$route.params.pid}});
          }
          
        }else{
          if(this.privacyoptions != 1){
            this.term_condition=true;
            this.$toastr.e(' Terms & Policy is Required');
            return false;
          }else if (this.userJobApply.title =='' || this.userJobApply.description =='') {
            return false;
          }
          this.currentpayment_arr = this.proposalData[0];
          this.$bvModal.show('payment-modal');
      ...
      async submitProposal() {
          this.errorMessageshow = true;
          this.$v.proposalData.$touch(); 
          if (!this.$v.proposalData.$invalid && this.privacyoptions == 1) {
              this.term_condition=false;
              // var response = await this.genericAPIPOSTRequest({
              //     requestUrl: "UpdateMilestone",
              //     params: { 
              ...
              // if(response){
                ...
                this.$router.push({ name: 'index', path:'/client/jobpostreview/', params: {pid: this.$route.params.pid},query:{current:3} });
              // }else{
              //   this.$toastr.e('Offer Not Sent');
              //   this.$router.push({ name: 'index', path:'/client/jobpostreview/', params: {pid: this.$route.params.pid}});
              // }
      ...
      //this function is for accept milstone
      async SendOffer(_i){
        this.errorMessageshow = true;
        if(this.privacyoptions != 1){
          this.term_condition=true;
          ...
          if(res){
            await this.genericAPIPOSTRequest({
                ...
                params: this.userJobApply
              });
            this.$toastr.s('Offer Successfully Submitted');
            this.$router.push({ name: 'index', path:'/client/jobpostreview/', params: {pid: this.$route.params.pid}, query:{current:3} });
          }else{
            this.$toastr.e('Offer submit Unsuccessfull');
            this.$router.push({ name: 'index', path:'/client/jobpostreview/', params: {pid: this.$route.params.pid} });
          }
        }
      },
        ...
        if (getPostedData.length > 0 ) {
          ...
          var userJobApply = await this.genericAPIPOSTRequest({
            ...
            params:{
              ...
              'user_id':this.decodeId(this.$route.params.uid)
            }
          });
          if (userJobApply) {
      ...
      async getMilestonedetails(_i){
        ...
        if (getMilestoneData.milestone.length > 0 ) {
          this.proposalData = getMilestoneData.milestone;
          this.jobApplicationData=getMilestoneData.proposal;
          // var jobs_id = this.jobDetails.jobs_id;
          this.proposalData.map((data,index)=>{
            if(index == 0){
              this.proposalRequiredIds.job_application_id = data.job_application_id
              this.proposalRequiredIds.agency_id = data.agency_id
              this.proposalRequiredIds.user_id = data.user_id
            }
            // data.acceptStatus = 7;
            data.job_id = this.jobDetails.jobs_id;
          })
          this.projectType = this.proposalData[0].milstone_type;
          this.canEditMilestone =(this.jobApplicationData.application_status == 1 || this.jobApplicationData.application_status == 3)? true:false;
          // this.canEditMilestone =(this.proposalData[0].acceptStatus == 7)? true:false;
      ...
      addMilestone(){
        this.proposalData.push({
          ...
          amount:'',
          job_id: this.job_id,
          user_id: this.proposalRequiredIds.user_id,
          created_by: this.proposalRequiredIds.agency_id,
          created_user_type : this.$store.state.accountType,
          job_application_id: this.proposalRequiredIds.job_application_id,
        });
      },
    ...
    async mounted() {
      ...
      if(this.$route.params.pid != undefined){
        if(this.$route.params.uid != undefined){
          ...
          // this.getMilestonedetails(this.$route.params.uid);
          this.getUserDetail(this.decodeId(this.$route.params.uid));
