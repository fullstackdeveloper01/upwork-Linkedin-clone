<template>
  <div class="container"
    ...
    <b-row class="mt-100 mb-50 freelancer-wrap contracts-wrap">
      <b-col cols="12">
        <h4 class="mb-2 fz-30 fw-500 text-black"><b>All Contract</b></h4>
      </b-col>
      ...
      <b-col cols="12">
        <b-card-group deck>
          <b-card class="py-4 px-4 bg-white">
            ...
            <b-row>
              <b-col cols="12">
                <div class="inner-wrap" v-if="contractData.length>0">
                  <div class="posted-job-wrapper border-bottom" v-for="(data,index) in contractData" :key="index">
                    <div>
                      <div class="d-flex align-items-center mt-4">
                        ...
                        <div class="ml-auto" v-if="data.application_status==2 || data.application_status == 6">
                          <b-dropdown size="lg" id="dropdown-right" right variant="link" toggle-class="text-decoration-none" no-caret>
                      ...
                      <b-row class="mt-2">
                        ...
                        <b-col xl="3" lg="3" sm="12">
                          <span class="d-flex">
                            <h4 class="fz-18 fw-600 text-black mr-2" v-if="data.application_status == 7">Pending: waiting You to approve</h4>
                            <h4 class="fz-18 fw-600 text-black mr-2" v-else-if="data.application_status == 2"> {{ (data.button == 'Submit Work For Payment' || data.button == 'Submit Revision For Payment')?'Active: '+data.milestoneName:(data.button == 'Funding Pending')?'Waiting: '+data.milestoneName:'' }}</h4>
                          ...
                          <p v-else class="fz-18 fw-600 text-black mr-2">Offer recived {{ data.time }}</p>
                        </b-col>
                        <b-col xl="3" lg="3" sm="12" class="text-right">
                          <b-button class="btn btn-theme" v-if="data.application_status == 2 ||data.application_status == 6" @click="$router.push({'name': $store.state.accountType+'contractDetail','path':'/'+$store.state.accountType+'/contract-detail/','params':{pid:data.uniq_id}})">{{data.button}}</b-button>
                          <b-button class="btn btn-theme" v-else @click="$router.push({name: $store.state.accountType+'Offer',path:'/'+$store.state.accountType+'/offer/',params:{pid:data.uniq_id,uid:encodedid(data.freelancer_id)}})">{{data.button}}</b-button>
                          ...
                          <!-- <b-button class="btn " :class="(data.application_status==2)?'btn-success':(data.application_status == 6)?'btn-danger':'btn-theme'" @click="$router.push({'name': $store.state.accountType+'contractDetail','path':'/'+$store.state.accountType+'/contract-detail/','params':{pid:data.uniq_id}})" v-if="data.application_status==2 || data.application_status==6">{{(data.application_status==2)?'Active Milestone':(data.application_status == 6)?'Ended':'Awaiting Milestone'}}</b-button>
                          <b-button class="btn " disabled :class="(data.application_status==2)?'btn-success':(data.application_status == 6)?'btn-danger':'btn-theme'"  v-else>{{(data.application_status==2)?'Active Milestone':(data.application_status == 6)?'Ended':'Awaiting Milestone'}}</b-button> -->
                          <!-- <p class="fz-14 text-theme fw-600 text-right mt-3 ">{{(data.paymentStatus == 1) ? 'Payment UnderProcess' :''}}</p> -->
                          <!-- <p class="fz-14 text-black fw-600 text-right mt-3">Dec 10, 2021 - Present</p> -->
                        </b-col>
                      </b-row>
                    </div>
                  </div>
                ...
                <div class="inner-wrap" v-else >  
                  <div class="posted-job-wrapper border-bottom" >
                    <div>
...
@import url('https://fonts.googleapis.com/css2?family=Source+Sans+Pro:wght@200;300;400;600;700;900&display=swap');
  ...
  .contract-wrap .inner-wrap{border: 1.5px solid #EBEBEB;}
  .fz-22{font-size:22px!important;}
  .lh-22{line-height:22px!important;}
  .text-muted {color: #999999!important;}
  .inner-wrap div:last-child{border-bottom:0px!important;}
  ...
  .contracts-wrap .input-group-text{border-radius: 0px 4px 4px 0;background-color: #fff;}
  .contracts-wrap .search-input .form-control{border-right: 0px;border-radius: 4px 0 0 4px!important;}
  .contracts-wrap .tags {margin-top:10px;margin-left:-32px;}
  .contracts-wrap svg {cursor:pointer;}
  .contracts-wrap .tags li {background: #FCFCFC;border: 1px solid #F2613C;border-radius: 100px;padding: 2px 15px;}
  ...
  .b-form-btn-label-control.form-control{margin-bottom:0px!important;}
