        ...
        <b-col xl="12" lg="12" md="12">
          <b-card no-body class="freelancer-wrap mt-12 bg-white">
            <ul class="nav nav-pills mb-3 py-2 px-4" id="pills-tab" role="tablist">
              <a href="javascript:void(0);" @click="$router.push('/freelancer/search')">
              
              <li class="nav-item" role="presentation">
                <button class="nav-link" id="pills-home-tab" data-bs-toggle="pill" data-bs-target="#pills-profile" type="button" role="tab" aria-controls="pills-home" aria-selected="true">Search </button>
              </li></a>
              <li class="nav-item" role="presentation">
                <button class="nav-link active" id="pills-home-tab" data-bs-toggle="pill" data-bs-target="#pills-profile" type="button" role="tab" aria-controls="pills-profile" aria-selected="false" v-on:click="filterByData()">Saved Jobs <span class="badge badge-secondary br-50" v-html="(proposalDetails.saved_job > 0)?proposalDetails.saved_job:''"></span></button>
              </li>
            </ul>
            <div class="tab-content" id="pills-tabContent">
              <div class="tab-pane fade show active" id="pills-home" role="tabpanel" aria-labelledby="pills-home-tab">
                    ...
                    <b-row>
                      <b-col xl="11" lg="11" md="12">
                        <div class="input-group search-bar">
                          <input type="text" class="form-control border-none" v-model="filtersValue.title" placeholder="Search for jobs" @keyup="filterByData()" aria-describedby="basic-addon2">
                      ...
                      <b-col xl="1" lg="1" md="12">
                        <div class="input-group search-bar">
                          <button class="btn btn-theme" @click="$refs['edit-category-modal'].show()">Filter</button>
                          <b-modal ref="edit-category-modal" size="lg" title="Filter By Skills" class="edit-category-modal" no-close-on-backdrop >
                            <div class="text-left">
                              <h5 class="text-black fz-20 fw-600" v-if="filterTags.length > 0">
                                Selected Skills
                              </h5>
                                  ...
                                  >
                                    <div class="d-flex align-items-center">
                                      ...
                                      <a href="javascript:" class="text-muted d-inline-flex">
                                        <div class="up-icon xs" @click="removeSkill(tagIndex)">
                ...
                <div v-if="dataStatus">
                  <div class="job-card py-2 px-4 pb-4 mb-2"  v-for="(data,index) in filteredAndSorted" :key="index" :class="(data.readStatus!=1)?'unread-bg':''" :id="data.jobs_id">
                    <div class="action-btn">
                      ...
                      <button type="button" v-if="data.action == 3" class="btn btn-secondary br-50 btn-archive saved-btn"  v-on:click="savePost(data)">
                      ...
                      <button type="button" v-else class="btn btn-secondary br-50 btn-archive"  v-on:click="savePost(data)">
                        <svg width="14" height="19" viewBox="0 0 14 19" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <path d="M1.82903 0.620117H11.997C12.6246 0.620117 13.1542 1.12649 13.1542 1.77727L13.1542 16.8113C13.1542 16.8115 13.1542 16.8116 13.1542 16.8118C13.1524 17.5271 12.2615 17.9031 11.7348 17.3707L11.7329 17.3688L7.8604 13.4974C7.35368 12.9738 6.47239 12.9738 5.96568 13.4974L2.0932 17.3688L2.09124 17.3707C1.56473 17.903 0.67387 17.5274 0.671875 16.8118V1.77727C0.671875 1.12649 1.20151 0.620117 1.82903 0.620117Z" stroke="black"/>
                        </svg>
                      </button>
                    </div>
                    <h5 class="text-black fw-600 cursor-pointer text-theme-hover" v-b-toggle.sidebar-right v-on:click="postDetailsclick(data.jobs_id)">{{data.job_title.toUpperCase()}}</h5>
                    ...
                    <ul class="d-md-flex align-items-center fz-14 post text-muted mb-2" v-b-toggle.sidebar-right v-on:click="postDetailsclick(data.jobs_id)">
                      ...
                      <li>Location- {{data.country_name}}</li> 
                      <li class="d-none d-sm-none d-md-block d-lg-block d-lg-block">|</li> 
                    ...
                    <div class="skill-slider-wrapper mt-5">
                      <!-- <ul class="inline-block align-items-center fz-14 post tags mb-0 w-100">
                        <slider-frame>
    ...
    export default {
      ...
      methods: {
        ...
        selectedSkills(skill) {
          ...
          this.filtersValue.category_ids.push(skill.qp_role_skill_master_id);
        },
        // this fun is for to remove selectd filter category
        removeSkill(index) {
          this.selectedTags.splice(index, 1);
          this.filtersValue.category_ids.splice(index, 1);
          this.filterByData();
        },
        ...
        async filterByData(){
          ...
          var  passApidata= {
              "filter": 3,
              "from": "search",
              "search":{"category_ids":this.filtersValue.category_ids,"title":this.filtersValue.title}
          };
          axios.post(this.$store.state.baseUrl+'/getJobsList',passApidata,{headers: this.headToken}).then((response)=>{
            if(response.data.status ==  true){
              this.postData = response.data.result;
      ...
      computed:{
        ...
        //this fun for to show selected tags
        filterTags() {
  ...
  <style>
    ...
    /*.freelancer-wrap .job-card:hover li{color:#F2613C!important;}*/
    ...
    .tt-menu .custom-control{margin-bottom: 5px;}
    .up-skill-container {
