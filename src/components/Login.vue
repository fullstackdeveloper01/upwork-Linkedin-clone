<template>
  <section class="login-wrapper">
    <div class="container-fluid">
      <b-row>
        <b-col xl="4" lg="4" md="12" class="fxt-bg-img fxt-bg-img1">
          <a @click="$router.push('/')" class="mt-8 fxt-logo d-block">
            <svg
              width="106px"
              height="24px"
              viewBox="0 0 106 24"
              version="1.1"
              xmlns="http://www.w3.org/2000/svg"
              xmlns:xlink="http://www.w3.org/1999/xlink"
            >
              <title>0DDEA7CC-09FA-43D7-91E5-0FB6CE4E5E4E</title>
              <g
                id="calendar-9Dec2020"
                stroke="none"
                stroke-width="1"
                fill="none"
                fill-rule="evenodd"
              >
                <g
                  id="Header/header"
                  transform="translate(-25.000000, -23.000000)"
                  fill="#FFFFFF"
                >
                  <path
                    d="M37.2171088,23 C40.6784385,23 43.5744946,24.1647612 45.9047399,26.4922442 C48.1732115,28.7147603 49.3481642,31.4316658 49.4297823,34.6412076 C49.4346831,34.8394428 49.2718155,35.0020449 49.0733162,35.0020449 L49.0733162,35.0020449 L43.9787986,35.0020449 C43.7939074,35.0020449 43.6317561,34.8510342 43.6242,34.6665013 C43.5543331,32.9586238 42.9458358,31.4993541 41.7983858,30.2895509 C40.5769865,28.9791096 39.049727,28.323084 37.2171088,28.323084 C35.3844905,28.323084 33.8572311,28.9791096 32.6358318,30.2895509 C31.4138953,31.5778468 30.8032135,33.149024 30.8032135,35.0020449 C30.8032135,36.8560677 31.4138953,38.4382996 32.6358318,39.7488125 C33.7821358,40.9579001 35.1978804,41.5998658 36.8825642,41.6742084 C37.0665959,41.6822938 37.2171088,41.8443236 37.2171088,42.0282841 L37.2171088,42.0282841 L37.2171088,46.643825 C37.2171088,46.8419528 37.0543845,47.0045192 36.8561,47 C33.5312811,46.9226278 30.7444838,45.7713542 28.4951351,43.5461191 C26.1648899,41.2181352 25,38.3702536 25,35.0020449 C25,31.6348381 26.1648899,28.7975819 28.4951351,26.4922442 C30.8254162,24.1647612 33.7325736,23 37.2171088,23 Z M65.6810811,23.3393359 C65.8110743,23.3393359 65.9582568,23.4371834 66.00875,23.5567828 L66.00875,23.5567828 L75.4832162,46.0364191 C75.5974527,46.3076731 75.4499122,46.5296278 75.1551892,46.5296278 L75.1551892,46.5296278 L69.82475,46.5296278 C69.6915338,46.5296278 69.5425608,46.4282385 69.4938581,46.304346 L69.4938581,46.304346 L68.0213176,42.5628857 L58.2475088,42.5628857 L56.80795,46.3018416 C56.7597128,46.4271294 56.6103459,46.5296278 56.4760196,46.5296278 L56.4760196,46.5296278 L51.1480514,46.5296278 C50.8530777,46.5296278 50.7056446,46.30753 50.8204541,46.0360255 L50.8204541,46.0360255 L60.3275081,23.5563535 C60.3780014,23.4370046 60.5254345,23.3393359 60.6551412,23.3393359 L60.6551412,23.3393359 Z M87.1478716,23.474963 C90.1796149,23.474963 92.5549459,24.2323064 94.2749392,25.7465281 C96.0393378,27.2612148 96.9217162,29.3179089 96.9217162,31.9172185 C96.9217162,34.6072561 96.0056757,36.7431584 94.1732365,38.3248895 C92.3404392,39.8844395 89.8512297,40.663964 86.7066824,40.663964 L86.7066824,40.663964 L83.5846959,40.663964 L83.5846959,46.1743 C83.5846959,46.3697446 83.4246216,46.5296278 83.2290946,46.5296278 L83.2290946,46.5296278 L78.2048378,46.5296278 C78.0089527,46.5296278 77.8492365,46.3697446 77.8492365,46.1743 L77.8492365,46.1743 L77.8492365,23.8302908 C77.8492365,23.6348819 78.0089527,23.474963 78.2048378,23.474963 L78.2048378,23.474963 Z M105.023196,23.474963 C105.218723,23.474963 105.378797,23.6348819 105.378797,23.8302908 L105.378797,23.8302908 L105.378797,46.1743 C105.378797,46.3697446 105.218723,46.5296278 105.023196,46.5296278 L105.023196,46.5296278 L99.9989392,46.5296278 C99.8030541,46.5296278 99.6433378,46.3697446 99.6433378,46.1743 L99.6433378,46.1743 L99.6433378,23.8302908 C99.6433378,23.6348819 99.8030541,23.474963 99.9989392,23.474963 L99.9989392,23.474963 Z M114.003473,23.474963 C114.093716,23.474963 114.215115,23.5308452 114.273845,23.5991774 L114.273845,23.5991774 L125.298561,36.4602413 L125.298561,23.8302908 C125.298561,23.6348819 125.458635,23.474963 125.65452,23.474963 L125.65452,23.474963 L130.644399,23.474963 C130.839926,23.474963 131,23.6348819 131,23.8302908 L131,23.8302908 L131,46.1743 C131,46.3697446 130.839926,46.5296278 130.644399,46.5296278 L130.644399,46.5296278 L126.749973,46.5296278 C126.660804,46.5296278 126.54048,46.4749262 126.482108,46.4081323 L126.482108,46.4081323 L115.525074,33.8831845 L115.525074,46.1743 C115.525074,46.3697446 115.365,46.5296278 115.169473,46.5296278 L115.169473,46.5296278 L110.144858,46.5296278 C109.949331,46.5296278 109.789257,46.3697446 109.789257,46.1743 L109.789257,46.1743 L109.789257,23.8302908 C109.789257,23.6348819 109.949331,23.474963 110.144858,23.474963 L110.144858,23.474963 Z M40.9961878,37.3183301 C41.0893676,37.2235593 41.2658074,37.1883557 41.3882804,37.2400521 L41.3882804,37.2400521 L47.9629304,40.0156303 C48.1582784,40.0980941 48.2363101,40.3184031 48.1404804,40.5073722 C47.5850905,41.6023701 46.8356419,42.6135446 45.8997622,43.5484804 C44.9502385,44.4876735 43.8896297,45.2389349 42.7488764,45.7945013 C42.5615142,45.8857303 42.3433547,45.8075596 42.2616345,45.615943 L42.2616345,45.615943 L39.5021966,39.1514596 C39.4506291,39.0310374 39.4836108,38.8560925 39.5755014,38.7626812 L39.5755014,38.7626812 Z M63.1342162,30.0182968 L60.1477736,37.6804912 L66.1211959,37.6804912 L63.1342162,30.0182968 Z M87.0458108,28.5267929 L83.5846959,28.5267929 L83.5846959,35.6464433 L87.0458108,35.6464433 C88.3127973,35.6464433 89.3251689,35.3076083 90.0832838,34.6294373 C90.8413986,33.9512663 91.2206351,33.0476704 91.2206351,31.9172185 C91.2206351,30.8774947 90.8413986,30.0520694 90.0832838,29.4419803 C89.3251689,28.8318554 88.3127973,28.5267929 87.0458108,28.5267929 L87.0458108,28.5267929 Z"
                    id="Combined-Shape"
                  ></path>
                </g>
              </g>
            </svg>
          </a>
          <div class="fxt-header">
            <p>
              Online community connecting clients with quality professionals
            </p>
          </div>
        </b-col>
        <b-col xl="8" lg="8" md="12" class="bg-white pos-rel">
          <div class="mobile-logo-wrap">
            <a
              href="javascript:void(0);"
              class="py-2 mt-1 text-center fxt-logo d-block"
            >
              <svg
                class="m-auto"
                width="106px"
                height="24px"
                viewBox="0 0 106 24"
                version="1.1"
                xmlns="http://www.w3.org/2000/svg"
                xmlns:xlink="http://www.w3.org/1999/xlink"
              >
                <title>0DDEA7CC-09FA-43D7-91E5-0FB6CE4E5E4E</title>
                <g
                  id="calendar-9Dec2020"
                  stroke="none"
                  stroke-width="1"
                  fill="#000"
                  fill-rule="evenodd"
                >
                  <g
                    id="Header/header"
                    transform="translate(-25.000000, -23.000000)"
                    fill="#FFFFFF"
                  >
                    <path
                      d="M37.2171088,23 C40.6784385,23 43.5744946,24.1647612 45.9047399,26.4922442 C48.1732115,28.7147603 49.3481642,31.4316658 49.4297823,34.6412076 C49.4346831,34.8394428 49.2718155,35.0020449 49.0733162,35.0020449 L49.0733162,35.0020449 L43.9787986,35.0020449 C43.7939074,35.0020449 43.6317561,34.8510342 43.6242,34.6665013 C43.5543331,32.9586238 42.9458358,31.4993541 41.7983858,30.2895509 C40.5769865,28.9791096 39.049727,28.323084 37.2171088,28.323084 C35.3844905,28.323084 33.8572311,28.9791096 32.6358318,30.2895509 C31.4138953,31.5778468 30.8032135,33.149024 30.8032135,35.0020449 C30.8032135,36.8560677 31.4138953,38.4382996 32.6358318,39.7488125 C33.7821358,40.9579001 35.1978804,41.5998658 36.8825642,41.6742084 C37.0665959,41.6822938 37.2171088,41.8443236 37.2171088,42.0282841 L37.2171088,42.0282841 L37.2171088,46.643825 C37.2171088,46.8419528 37.0543845,47.0045192 36.8561,47 C33.5312811,46.9226278 30.7444838,45.7713542 28.4951351,43.5461191 C26.1648899,41.2181352 25,38.3702536 25,35.0020449 C25,31.6348381 26.1648899,28.7975819 28.4951351,26.4922442 C30.8254162,24.1647612 33.7325736,23 37.2171088,23 Z M65.6810811,23.3393359 C65.8110743,23.3393359 65.9582568,23.4371834 66.00875,23.5567828 L66.00875,23.5567828 L75.4832162,46.0364191 C75.5974527,46.3076731 75.4499122,46.5296278 75.1551892,46.5296278 L75.1551892,46.5296278 L69.82475,46.5296278 C69.6915338,46.5296278 69.5425608,46.4282385 69.4938581,46.304346 L69.4938581,46.304346 L68.0213176,42.5628857 L58.2475088,42.5628857 L56.80795,46.3018416 C56.7597128,46.4271294 56.6103459,46.5296278 56.4760196,46.5296278 L56.4760196,46.5296278 L51.1480514,46.5296278 C50.8530777,46.5296278 50.7056446,46.30753 50.8204541,46.0360255 L50.8204541,46.0360255 L60.3275081,23.5563535 C60.3780014,23.4370046 60.5254345,23.3393359 60.6551412,23.3393359 L60.6551412,23.3393359 Z M87.1478716,23.474963 C90.1796149,23.474963 92.5549459,24.2323064 94.2749392,25.7465281 C96.0393378,27.2612148 96.9217162,29.3179089 96.9217162,31.9172185 C96.9217162,34.6072561 96.0056757,36.7431584 94.1732365,38.3248895 C92.3404392,39.8844395 89.8512297,40.663964 86.7066824,40.663964 L86.7066824,40.663964 L83.5846959,40.663964 L83.5846959,46.1743 C83.5846959,46.3697446 83.4246216,46.5296278 83.2290946,46.5296278 L83.2290946,46.5296278 L78.2048378,46.5296278 C78.0089527,46.5296278 77.8492365,46.3697446 77.8492365,46.1743 L77.8492365,46.1743 L77.8492365,23.8302908 C77.8492365,23.6348819 78.0089527,23.474963 78.2048378,23.474963 L78.2048378,23.474963 Z M105.023196,23.474963 C105.218723,23.474963 105.378797,23.6348819 105.378797,23.8302908 L105.378797,23.8302908 L105.378797,46.1743 C105.378797,46.3697446 105.218723,46.5296278 105.023196,46.5296278 L105.023196,46.5296278 L99.9989392,46.5296278 C99.8030541,46.5296278 99.6433378,46.3697446 99.6433378,46.1743 L99.6433378,46.1743 L99.6433378,23.8302908 C99.6433378,23.6348819 99.8030541,23.474963 99.9989392,23.474963 L99.9989392,23.474963 Z M114.003473,23.474963 C114.093716,23.474963 114.215115,23.5308452 114.273845,23.5991774 L114.273845,23.5991774 L125.298561,36.4602413 L125.298561,23.8302908 C125.298561,23.6348819 125.458635,23.474963 125.65452,23.474963 L125.65452,23.474963 L130.644399,23.474963 C130.839926,23.474963 131,23.6348819 131,23.8302908 L131,23.8302908 L131,46.1743 C131,46.3697446 130.839926,46.5296278 130.644399,46.5296278 L130.644399,46.5296278 L126.749973,46.5296278 C126.660804,46.5296278 126.54048,46.4749262 126.482108,46.4081323 L126.482108,46.4081323 L115.525074,33.8831845 L115.525074,46.1743 C115.525074,46.3697446 115.365,46.5296278 115.169473,46.5296278 L115.169473,46.5296278 L110.144858,46.5296278 C109.949331,46.5296278 109.789257,46.3697446 109.789257,46.1743 L109.789257,46.1743 L109.789257,23.8302908 C109.789257,23.6348819 109.949331,23.474963 110.144858,23.474963 L110.144858,23.474963 Z M40.9961878,37.3183301 C41.0893676,37.2235593 41.2658074,37.1883557 41.3882804,37.2400521 L41.3882804,37.2400521 L47.9629304,40.0156303 C48.1582784,40.0980941 48.2363101,40.3184031 48.1404804,40.5073722 C47.5850905,41.6023701 46.8356419,42.6135446 45.8997622,43.5484804 C44.9502385,44.4876735 43.8896297,45.2389349 42.7488764,45.7945013 C42.5615142,45.8857303 42.3433547,45.8075596 42.2616345,45.615943 L42.2616345,45.615943 L39.5021966,39.1514596 C39.4506291,39.0310374 39.4836108,38.8560925 39.5755014,38.7626812 L39.5755014,38.7626812 Z M63.1342162,30.0182968 L60.1477736,37.6804912 L66.1211959,37.6804912 L63.1342162,30.0182968 Z M87.0458108,28.5267929 L83.5846959,28.5267929 L83.5846959,35.6464433 L87.0458108,35.6464433 C88.3127973,35.6464433 89.3251689,35.3076083 90.0832838,34.6294373 C90.8413986,33.9512663 91.2206351,33.0476704 91.2206351,31.9172185 C91.2206351,30.8774947 90.8413986,30.0520694 90.0832838,29.4419803 C89.3251689,28.8318554 88.3127973,28.5267929 87.0458108,28.5267929 L87.0458108,28.5267929 Z"
                      id="Combined-Shape"
                    ></path>
                  </g>
                </g>
              </svg>
            </a>
          </div>
          <div class="mt-4 header-menu">
            <div class="d-md-flex align-items-center">
              <label class="mr-2">Don’t have an account?</label>
              <router-link
                to="/sign-up"
                type="submit"
                v-if="$route.path === '/login' || $route.path === '/'"
                class="btn btn-outline-theme"
              >
                Get Started
              </router-link>
            </div>
          </div>
          <div class="fxt-bg-color">
            <div class="fxt-content">
              <div class="fxt-form">
                <h2 class="mb-0 title">Sign in to Qapin.</h2>
                <h5 class="mb-4 subtitle">Enter your details below.</h5>
                <form action="" @submit.prevent="submitLoginData">
                  <div class="form-group pos-rel">
                    <label>Email Address</label>
                    <v-text-field
                      v-model="email"
                      placeholder="Enter your email address"
                      :error-messages="emailErrors"
                      @input="$v.email.$touch()"
                      @blur="$v.email.$touch()"
                    />
                  </div>
                  <div class="form-group pos-rel">
                    <label>Password</label>
                    <v-text-field
                      v-model="password"
                      placeholder="Enter your passowrd"
                      :error-messages="passwordErrors"
                      name="input-10-1"
                      type="password"
                      @input="$v.password.$touch()"
                      @blur="$v.password.$touch()"
                      @click:append="showPassword = !showPassword"
                    >
                      <div @click="showPassword = !showPassword" class="z-10">
                        <EyeIcon
                          class="absolute w-4 h-4 text-gray-500 cursor-pointer hover:text-primary-500"
                          v-if="showPassword"
                        />
                        <EyeOffIcon
                          v-else
                          class="absolute w-4 h-4 text-gray-500 cursor-pointer hover:text-primary-500"
                        />
                      </div>
                    </v-text-field>
                  </div>
                  <div class="text-right form-group">
                    <router-link to="/forgot-password" class="text-theme fz-14">
                      Forgot Password?
                    </router-link>
                  </div>
                  <div class="text-right form-group">
                    <button type="submit" class="mt-4 btn btn-theme">
                      Sign In
                    </button>
                  </div>
                  <div class="text-right form-group" v-if="showResendLink">Resend 
                    <a href="javascript:void(0);" class="text-theme fz-14" @click="resendVerificationMail()">
                      verification email
                    </a>
                  </div>
                </form>
              </div>
            </div>
          </div>
        </b-col>
      </b-row>
    </div>
  </section>
</template>
<script>
import { validationMixin } from "vuelidate";
import { required, email } from "vuelidate/lib/validators";
import axios from "axios";
import { EyeIcon, EyeOffIcon } from "@vue-hero-icons/outline";
import { mapActions } from "vuex";
let base_url = "";
export default {
  title: "Login | Qapin",

  mixins: [validationMixin],

  validations: {
    email: { required, email },
    password: { required },
  },

  components: {
    EyeIcon,
    EyeOffIcon,
  },

  data() {
    return {
      isSaving: false,
      email: "",
      password: null,
      showPassword: false,
      currentUser: null,
      showResendLink:false,
    };
  },

  computed: {
    emailErrors() {
      const errors = [];
      if (!this.$v.email.$dirty) return errors;
      !this.$v.email.email && errors.push("Must be valid e-mail");
      !this.$v.email.required && errors.push("E-mail is required");
      return errors;
    },
    passwordErrors() {
      const errors = [];
      if (!this.$v.password.$dirty) return errors;
      !this.$v.password.required && errors.push("Password is required");
      return errors;
    },
  },
  async mounted() {
    console.log('token',this.$store.state.currentUserData.token);
    // if (localStorage.getItem("token")) {
    // var accountType = localStorage.getItem("accountType"); 
    //   if(accountType=='agency'){
    //     this.$router.push("/agency");
    //   }else if(accountType=='freelancer'){
    //     this.$router.push("/freelancer");
    //   }else if(accountType=='client'){
    //     this.$router.push("/client/dashboard");
    //   }
    // },
    if (this.$store.state.currentUserData.token) {
      if(this.$store.state.currentUserData.updateProfile_afterVerification == '0' && this.$store.state.currentUserData.isQualityProfessional=='1'){
        this.$router.push("/update-profile");
        // console.log('sadsada');
        // return false;
      }else{
        // this.getUserLoginSess();
        if(this.$store.state.currentUserData.isCompanyLogin=='1'){
          this.$router.push("/agency/search");
        }
        if(this.$store.state.currentUserData.isQualityProfessional=='1'){
          this.$router.push("/freelancer");
        }
        if(this.$store.state.currentUserData.isClientLogin=='1'){
          this.$router.push("/client/dashboard");
        }
      }
      
    }
  },
  methods: {
    ...mapActions(["genericAPIPOSTRequest"]),
    // this fun is for to resend verification email
    async resendVerificationMail(){
      var res = await this.genericAPIPOSTRequest({
        requestUrl:'resendMail',
        params:{
          login_name:this.email,
        }
      });
      // console.log(res);
      if(res){
        this.showResendLink = false;
        this.$toastr.s('Verification mail Successfully send');
      }
    },
    async submitLoginData() {
      var that = this;
      this.isSaving = true;
      this.$v.$touch();

      const loginData = {
        login_name: this.email,
        password: this.password,
      };

       if (!this.$v.$invalid) {
        axios.post(base_url + "user/login", loginData).then((response) => {
          if (response.data.status === true) {
            localStorage.setItem(
              "loginUserUuid", response.data.result.uuid
            );
            localStorage.setItem(
              "token",
              "Bearer " + response.data.result.token
            );
            localStorage.setItem(
              "loginUserData",
              JSON.stringify(response.data.result)
            );
            this.$store.dispatch("currentUserData", response.data.result);
            this.$store.dispatch("currentUserLoginUuid", response.data.result.uuid);
            this.switchaccount(response.data.result.profileType, response.data.result.uuid,response.data.result);
            this.resetLoginForm();
            this.getUserLoginSess();
            this.isSaving = true;
            
            setTimeout(function () {
              that.$toastr.s("Login successfully");
              window.location.reload();
            }, 1000);
          } else {
            this.isSaving = false;
            this.$toastr.e(response.data.message);
            if (response.data.message == 'Your account not verified.') {
              this.showResendLink = true;
            }
            setTimeout(function () {
              that.$toastr.removeByType("error");
            }, 1000);
          }
        })
        .catch((error) => {
          console.log("error =>", error);
          this.$toastr.e("Something went wrong please try again!");
        });
      }
    },
    async switchaccount(accountType,uuid,_v) {
      var res = await this.genericAPIPOSTRequest({
        requestUrl: "user/switchAccount",
        params: { account: accountType, 'uuid': uuid, data: this.getLoginUserIdRequest() },
      });
      localStorage.setItem(
        "token",
        "Bearer " + res.token
      );
      localStorage.setItem(
        "loginUserData",
        JSON.stringify(res)
      );
      res.isClientLogin         = _v.isClientLogin;
      res.isCompanyLogin        = _v.isCompanyLogin;
      res.isQualityProfessional = _v.isQualityProfessional; 
      this.$store.dispatch("currentUserData", res);

      // this.$store.commit("setcurrentUserData", res);
    },
    resetLoginForm() {
      this.$v.$reset();
      this.email = "";
      this.password = "";
    },
    async getUserLoginSess() {
      var res = await this.genericAPIPOSTRequest({
        requestUrl: "user/getUserLoginSess",
        params: {},
      });
      if(res.isQualityProfessional=='1'){
        this.$store.dispatch("accountType", 'freelancer'); 
        localStorage.setItem("accountType","freelancer"); 
      }
      if(res.isClientLogin=='1'){
        this.$store.dispatch("accountType", 'client'); 
        localStorage.setItem("accountType","client"); 
      }
      if(res.isCompanyLogin=='1'){
        this.$store.dispatch("accountType", 'agency'); 
        localStorage.setItem("accountType","agency"); 
      } 
    },
     
  },
};
</script>
<style>
navbar-main {
  display: none !important;
}
/* .relative {
  padding: 0px !important;
} */
.fxt-bg-img1 {
  background-image: url(http://qapin_mvp5.manageprojects.in/upload/freelancer/login-bg.svg) !important;
}
.fxt-bg-img {
  background-repeat: no-repeat;
  background-size: cover;
  background-color: #fff;
  position: relative;
}
.login-wrapper .fxt-bg-color {
  background-color: #ffffff;
  min-height: 100vh;
  display: -webkit-box;
  display: -ms-flexbox;
  display: flex;
  -webkit-box-align: center;
  -ms-flex-align: center;
  align-items: center;
  -webkit-box-pack: center;
  -ms-flex-pack: center;
  justify-content: center;
  /*padding: 60px 30px;*/
  padding:100px 7px;
}
.login-wrapper .fxt-content {
  max-width: 500px;
  width: 100%;
}
.login-wrapper .title {
  font-family: "Source Sans Pro";
  font-style: normal;
  font-weight: 600;
  font-size: 36px;
  line-height: 45px;
  color: #000000;
}
.login-wrapper .subtitle {
  font-family: "Source Sans Pro";
  font-style: normal;
  font-weight: 600;
  font-size: 20px;
  line-height: 25px;
  color: #979797;
}
.login-wrapper label {
  font-family: "Source Sans Pro";
  font-style: normal;
  font-weight: 400;
  font-size: 16px;
  line-height: 20px;
  color: #000000;
}
.login-wrapper input {
  font-family: "Source Sans Pro";
  font-style: normal;
  font-weight: 400;
  font-size: 16px;
  line-height: 20px;
  background: #ffffff;
  border: 1px solid #cecece;
  border-radius: 6px;
}
.login-wrapper .btn {
  padding: 8px 45px !important;
}
.login-wrapper .btn:hover {
  padding: 8px 45px !important;
}
.login-wrapper .fxt-header {
  position: absolute;
  bottom: 20px;
  left: 20%;
  right: 20%;
}
.login-wrapper .fxt-header p {
  font-family: "Source Sans Pro";
  font-style: normal;
  font-weight: 600;
  font-size: 32px;
  line-height: 30px;
  text-align: center;
  color: #ffffff;
}
.theme--light.v-text-field > .v-input__control > .v-input__slot:before {
  border: 0px;
}
.v-text-field {
  margin-top: 0px !important;
  padding-top: 5px !important;
}
.v-input input {
  padding: 18px 12px !important;
  max-height: 43px !important;
}
.v-text-field__details {
  position: absolute;
  bottom: -8px;
  right: 0;
}
.pos-rel {
  position: relative;
}
.header-menu {
  position: absolute;
  right: 20px;
}
.v-text-field > .v-input__control > .v-input__slot:after {
  border-color: transparent !important;
  border-style: none !important;
}
@media only screen and (min-width: 320px) and (max-width: 815px) {
  .fxt-bg-img {
    display: none;
  }
  .mobile-logo-wrap {
    display: block;
    background-color: #f2613c;
  }
}
p {
  margin-top: 0 !important;
  margin-bottom: 1rem !important;
}
h6,
.h6,
h5,
.h5,
h4,
.h4,
h3,
.h3,
h2,
.h2,
h1,
.h1 {
  margin-top: 0 !important;
  margin-bottom: 0.5rem !important;
  font-weight: 500 !important;
  line-height: 1.2 !important;
}
.v-messages__wrapper div {
  color: #c10000 !important;
}

</style>
